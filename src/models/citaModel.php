...
class Cita {
    ...
    public function getCitasByPeluqueraFull($id_peluquera) {
        $query = "
            SELECT
                c.id,
                c.fecha,
            ...
            JOIN usuarios  d ON m.id_dueno     = d.id
            JOIN usuarios  p ON c.id_peluquera = p.id
            WHERE c.id_peluquera = :id_peluquera
            ORDER BY c.fecha ASC, c.hora ASC";
    ...
    public function getCitasByUsuario($userId) {
        $query = "
            SELECT
                ...
                u.telefono AS telefono_peluquera
            FROM citas c
            JOIN mascotas m ON c.id_mascota   = m.id
    ...
    public function getAllMascotasActivas() {
        $query = "
            SELECT
                ...
                u.nombre AS dueno
            FROM mascotas m
            JOIN usuarios u ON m.id_dueno = u.id
            WHERE m.activa = 1
